...
;; Originally from Greg Hendershott's excellent Markdown package:
                    ...
                    [`(tr ([,_ ,_] ...) ,cells ...)
                     (for/list ([cell cells])
                       (match cell
                         ...
                         [_ ""]))])))]
      [`(p () ,es ...)
        ;; NOTE(joe): Some failed attempts for posterity if anyone tries to play with indentation:
        #;(map (lambda (c) (core:paragraph (core:style #f '()) c)) (xs->ps es)) ;; more weirdness with quotes
        #;(para (xs->ps es)) ;; no indents
        #;(map para (xs->ps es)) ;; weird breaks with quotes/mdashes
        #;(xs->ps es) ;; no line breaks at all
        #;(nested (map nested (xs->ps es))) ;; latex error
        #;(apply nested (map para (map list (xs->ps es)))) ;; I forget, but didn't do what I wanted
        (core:paragraph (core:style #f '(never-indents)) (xs->ps es))]
