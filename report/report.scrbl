...
reviewed using all the facilities for normal
assignments''@~cite["hamer-aropa"].  Based on this observation, Aropä's main
workflow suggests two explicit kinds of meta-review.  First reviewees can
@emph{rate} their reviews, which gives feedback on the perceived review
quality.  Second, there is an explicit @emph{dispute} phase, in which the
reviewee can disagree with the content of a review and request that the
reviewer reconsider, after which the reviewer can submit revised feedback.
This review-dispute-revise loop is, in effect, an in-flow review of the review,
since the student writing the review gets feedback on multiple versions of the
review.
...
peer review, but found the fit to be problematic unless the process
matched that of conference reviews.
...
papers, not just the subset that they are due to review (this sets
aside conflict-of-interest, which is less of an issue in the pedagogic
context).  These are
...
@section{Industrial Code-Review Practice}
...
Given the rich history of code-review in industry, it is worth
asking what @IFPR can learn from this practice.  
Code review is an essential component of industrial
software-development.  Industrial peer review is inherently
...
@subsection{Motivations for Industrial Code Review}
...
@subsection{What and How to Review}
